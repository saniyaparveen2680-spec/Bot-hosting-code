...
# ============================================
# 🔥 CONFIG - ALPHANOIRR HOSTING BOT
# ============================================
TOKEN = ''
OWNER_ID = 8742883635
ADMIN_ID = 8742883635
YOUR_USERNAME = '@YOUR_FATHER_786K'
UPDATE_CHANNEL = 'https://t.me/Altafhost'
BOT_NAME = "ALTAF HOSTING BOT"

# ============================================
# 🎨 COLORFUL BUTTONS (Bot API 9.4 — blue/red/green)
# Falls back to a plain button automatically on older
# pyTelegramBotAPI versions that don't support "style" yet.
# ============================================
_STYLE_MAP = {'primary': 'primary', 'danger': 'danger', 'success': 'success'}

def cbtn(text, callback_data=None, url=None, style=None):
    real_style = _STYLE_MAP.get(style)
    kwargs = {}
    ...
    try:
        if real_style:
            return types.InlineKeyboardButton(text, style=real_style, **kwargs)
...
load_data()
# ============================================
# 🔥 PART 2 - HELPERS + SCRIPT RUNNER
# ALPHANOIRR HOSTING BOT
# ============================================
...
def is_bot_running(script_owner_id, file_name):
    ...
    if script_info and script_info.get('process'):
        try:
            ...
            if not is_running:
                logger.warning(f"Process {script_info['process'].pid} for {script_key} not running/zombie. Cleaning up.")
                ...
                try:
                    ...
                    for child in children:
                        try:
                            ...
                            logger.error(f"Error terminating child {child.pid}: {e}. Trying kill...")
                            try: child.kill(); logger.info(f"Killed child process {child.pid}")
                    ...
                    for p in alive:
                        ...
                        try: p.kill()
                        except Exception as e: logger.error(f"Failed to kill child {p.pid}: {e}")
                    ...
                    try:
                        ...
                        logger.error(f"Error terminating parent {pid}: {e}. Trying kill...")
                        try: parent.kill(); logger.info(f"Killed parent process {pid}")
                        except Exception as e2: logger.error(f"Failed to kill parent {pid}: {e2}")

                except psutil.NoSuchProcess:
                    logger.warning(f"Process {pid or 'N/A'} for {script_key} not found during kill.")
...
TELEGRAM_MODULES = {
    ...
    'threading': None, 'subprocess': None, 'zipfile': None,
    'tempfile': None, 'shutil': None, 'sqlite3': None,
    'atexit': None
}
...
def create_main_menu_inline(user_id):
    ...
    buttons = [
        cbtn('📢 Updates Channel', url=UPDATE_CHANNEL, style='primary'),
        cbtn('📤 Upload File', callback_data='upload', style='success'),
        cbtn('📂 Check Files', callback_data='check_files', style='primary'),
        cbtn('⚡ Bot Speed', callback_data='speed', style='primary'),
        cbtn('📞 Contact Owner', url=f'https://t.me/{YOUR_USERNAME.replace("@", "")}', style='success')
    ]

    if user_id in admin_ids:
        admin_buttons = [
            cbtn('💳 Subscriptions', callback_data='subscription', style='primary'),
            cbtn('📊 Statistics', callback_data='stats', style='primary'),
            cbtn('🔒 Lock Bot' if not bot_locked else '🔓 Unlock Bot',
                 callback_data='lock_bot' if not bot_locked else 'unlock_bot', style='danger'),
            cbtn('📢 Broadcast', callback_data='broadcast', style='success'),
            cbtn('👑 Admin Panel', callback_data='admin_panel', style='danger'),
            cbtn('🟢 Run All User Scripts', callback_data='run_all_scripts', style='success')
        ]
        ...
        markup.add(cbtn('📊 Statistics', callback_data='stats', style='primary'))
        markup.add(buttons[4])
    return markup
...
def create_control_buttons(script_owner_id, file_name, is_running=True):
    ...
    if is_running:
        markup.row(
            cbtn("🔴 Stop", callback_data=f'stop_{script_owner_id}_{file_name}', style='danger'),
            cbtn("🔄 Restart", callback_data=f'restart_{script_owner_id}_{file_name}', style='primary')
        )
        markup.row(
            cbtn("🗑️ Delete", callback_data=f'delete_{script_owner_id}_{file_name}', style='danger'),
            cbtn("📜 Logs", callback_data=f'logs_{script_owner_id}_{file_name}', style='primary')
        )
    else:
        markup.row(
            cbtn("🟢 Start", callback_data=f'start_{script_owner_id}_{file_name}', style='success'),
            cbtn("🗑️ Delete", callback_data=f'delete_{script_owner_id}_{file_name}', style='danger')
        )
        markup.row(
            cbtn("📜 View Logs", callback_data=f'logs_{script_owner_id}_{file_name}', style='primary')
        )
    markup.add(cbtn("🔙 Back to Files", callback_data='check_files', style='primary'))
    return markup

def create_admin_panel():
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.row(
        cbtn('➕ Add Admin', callback_data='add_admin', style='success'),
        cbtn('➖ Remove Admin', callback_data='remove_admin', style='danger')
    )
    markup.row(cbtn('📋 List Admins', callback_data='list_admins', style='primary'))
    markup.row(cbtn('🔙 Back to Main', callback_data='back_to_main', style='primary'))
    return markup

def create_subscription_menu():
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.row(
        cbtn('➕ Add Subscription', callback_data='add_subscription', style='success'),
        cbtn('➖ Remove Subscription', callback_data='remove_subscription', style='danger')
    )
    markup.row(cbtn('🔍 Check Subscription', callback_data='check_subscription', style='primary'))
    markup.row(cbtn('🔙 Back to Main', callback_data='back_to_main', style='primary'))
    return markup
    # ============================================
# 🔥 PART 5 - FILE HANDLERS + COMMANDS
# ALPHANOIRR HOSTING BOT
# ============================================
...
def _logic_updates_channel(message):
    ...
    markup.add(cbtn('📢 Updates Channel', url=UPDATE_CHANNEL, style='primary'))
...
def _logic_upload_file(message):
    ...
    # ============================================
# 🔥 PART 6 - COMMAND HANDLERS + CALLBACKS
# ALPHANOIRR HOSTING BOT
# ============================================
...
def _logic_check_files(message):
    ...
    for file_name, file_type in sorted(user_files_list):
        ...
        markup.add(cbtn(btn_text, callback_data=f'file_{user_id}_{file_name}', style='success' if is_running else 'danger'))
...
def _logic_contact_owner(message):
    markup = types.InlineKeyboardMarkup()
    markup.add(cbtn('📞 Contact Owner', url=f'https://t.me/{YOUR_USERNAME.replace("@", "")}', style='success'))
...
def ping(message):
    ...
    # ============================================
# 🔥 PART 7 - DOCUMENT HANDLER + CALLBACKS
# ALPHANOIRR HOSTING BOT
# ============================================
...
def check_files_callback(call):
    ...
    if not user_files_list:
        ...
        try:
            markup = types.InlineKeyboardMarkup()
            markup.add(cbtn("🔙 Back to Main", callback_data='back_to_main', style='primary'))
...
def process_broadcast_message(message):
    ...
    if not broadcast_content and not (message.photo or message.video or message.document or
                                      message.sticker or message.voice or message.audio):
    ...
    markup = types.InlineKeyboardMarkup()
    markup.row(
        cbtn("✅ Confirm & Send", callback_data=f"confirm_broadcast_{message.message_id}", style='success'),
        cbtn("❌ Cancel", callback_data="cancel_broadcast", style='danger')
    )
...
def handle_confirm_broadcast(call):
    ...
    try:
        ...
        bot.edit_message_text("❌ Unexpected error during broadcast confirm.", chat_id, call.message.message_id,
                              reply_markup=None)
            ...
            if any(s in err_desc for s in ["bot was blocked", "user is deactivated", "chat not found",
                ...
                try:
                    if broadcast_text:
                        ...
                        bot.send_photo(user_id_bc, photo_id, caption=caption,
                                       parse_mode='Markdown' if caption else None)
                    elif video_id:
                        bot.send_video(user_id_bc, video_id, caption=caption,
                                       parse_mode='Markdown' if caption else None)
                    sent_count += 1
...
def list_admins_callback(call):
    bot.answer_callback_query(call.id)
    try:
        admin_list_str = "\n".join(f"- `{aid}` {'(Owner)' if aid == OWNER_ID else ''}"
        ...
        if sub_user_id_check in user_subscriptions:
            ...
            if expiry_dt:
                if expiry_dt > datetime.now():
                    ...
                    bot.reply_to(message,
                                 f"⚠️ User `{sub_user_id_check}` expired sub (On: {expiry_dt:%Y-%m-%d %H:%M:%S}).")
